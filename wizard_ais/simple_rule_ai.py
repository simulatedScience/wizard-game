...
from scoring_functions import update_winning_card
...
class Simple_Rule_Ai():
  ...
  def get_trump_color_choice(self, hands: list, game_state: Wizard_Game_State) -> int:
    ...
    --------
        int: integer representing a card color
            ...
            3 -> blue
    """
    hand = hands[game_state.trick_active_player]
  ...
  def _get_winning_actions(self,
                           valid_actions: list,
                           card_values: list,
                           game_state: Wizard_Game_State):
    if game_state.cards_to_be_played == game_state.get_state:
      # all actions win.
  ...
  def _get_card_values(self, actions: list, game_state: Wizard_Game_State):
    """
